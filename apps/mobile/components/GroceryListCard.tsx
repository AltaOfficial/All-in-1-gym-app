...
export default function GroceryListCard() {
  ...
  useEffect(() => {
    getGroceryList({ dateFrom: getCurrentWeek()?.weekFrom ?? new Date(), dateTo: getCurrentWeek()?.weekTo ?? new Date() })
    .then((groceryList) => {
  ...
  return (
    <View className='mb-4 px-4 flex-1'>
        ...
        <View className={`bg-gray1 h-64 rounded-xl p-4 pt-5 flex-col items-center justify-between`}>
            <View className='flex-row items-start gap-4 justify-between w-full'>
                <View className='flex-col gap-2'>
                    {groceryList?.slice(0, 4).map(item => (
                        <View className='flex-row items-center' key={item.id}>
                            ...
                            <Text className='text-white font-[HelveticaNeue]'>{item.itemName}</Text>
                        </View>
                    ))}
                </View>
                <View className='flex-col items-center place-self-end place-items-end justify-end align-top'>
                    ...
                    <Text className='text-white font-[HelveticaNeue]'>${groceryList?.reduce((acc, item) => acc + item.cost * item.quantity, 0) ?? 0}</Text>
                </View>
            </View>

            <TouchableOpacity onPress={() => router.push('/(grocery)/groceryList')} className='flex-row items-center justify-center mt-4 border-gray2 border rounded-xl py-3 w-full'>
