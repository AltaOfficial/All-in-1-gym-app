...
const AddDailyMetrics = () => {
  ...
  const handleComplete = async () => {
    try {
      ...
      const hasMetrics = Object.values(requestData).some(
        (value) => value !== null
      );
      if (!hasMetrics) {
        return;
      ...
      const response = await fetch(
        `${process.env.EXPO_PUBLIC_BACKEND_URL}/bodymetrics/log`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        console.error(
          "Error logging body metrics:",
          errorData.message || "Failed to log body metrics"
        );
      }
    } catch (error) {
  ...
  return (
    <SafeAreaView edges={["bottom"]} className="flex-1">
      <ScrollView>
        ...
        <View className="px-4">
          <Text className="text-white text-2xl font-[HelveticaNeue] font-bold">
            Body Composition
          </Text>
