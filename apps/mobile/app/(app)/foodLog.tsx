...
const FoodLog = () => {
  ...
  const dinnerCalories = foodLog.filter(item => item.mealType === MealType.DINNER).reduce((acc, item) => acc + (item.calories || 0), 0);
  const lunchCalories = foodLog.filter(item => item.mealType === MealType.LUNCH).reduce((acc, item) => acc + (item.calories || 0), 0);
  const breakfastCalories = foodLog.filter(item => item.mealType === MealType.BREAKFAST).reduce((acc, item) => acc + (item.calories || 0), 0);
  ...
  return (
    <SafeAreaView edges={['bottom']} className="flex-1 bg-black">
      ...
      <ScrollView className="flex-1 flex-col">
        ...
        <Separator className="h-[0.4px]"/>
        <View>
            {foodLog.filter(item => item.mealType === MealType.BREAKFAST).map((item, index) => (
                <View key={index} className='flex-row justify-between items-center px-4 my-3'>
                    ...
                    <View className='flex-col items-end gap-2'>
                        ...
                        <Text className='text-gray3 font-[HelveticaNeue]'>{new Date(new Date().setHours(parseInt(item.time.split(':')[0]), parseInt(item.time.split(':')[1]))).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true })}</Text>
                    </View>
                </View>
            ))}
        </View>
        ...
        <Separator className="h-[0.4px]"/>
        <View>
            {foodLog.filter(item => item.mealType === MealType.LUNCH).map((item, index) => (
                <View key={index} className='flex-row justify-between items-center px-4 my-3'>
                    <View>
                        <Text className='text-white text-lg font-[HelveticaNeue]'>{item.foodName.length > 28 ? item.foodName.substring(0,25) + '...' : item.foodName}</Text>
                        <Text className='text-gray3 font-[HelveticaNeue]'>{item.servingSize} {item.servings ? `(${item.servings} servings)` : ''}</Text> 
        ...
        <Separator className="h-[0.4px]"/>
        <View>
            {foodLog.filter(item => item.mealType === MealType.DINNER).map((item, index) => (
                <View key={index} className='flex-row justify-between items-center px-4 my-3'>
                    <View>
                        <Text className='text-white text-lg font-[HelveticaNeue]'>{item.foodName.length > 28 ? item.foodName.substring(0,25) + '...' : item.foodName}</Text>
                        <Text className='text-gray3 font-[HelveticaNeue]'>{item.servingSize} {item.servings ? `(${item.servings} servings)` : ''}</Text> 
