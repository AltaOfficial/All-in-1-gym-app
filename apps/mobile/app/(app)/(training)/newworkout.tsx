import { StyleSheet, Text, View, TextInput, Pressable, ScrollView, Image, TouchableOpacity, Dimensions } from "react-native";
...
const SwipeableExerciseItem = ({ exercise, index, onDelete }: { 
  exercise: any, 
  index: number, 
  ...
  const panGesture = Gesture.Pan()
    ...
    .onEnd((event) => {
      ...
      if (Math.abs(event.translationX) > threshold) {
        ...
        translateX.value = withSpring(direction, { damping: 20, stiffness: 300 });
...
const NewWorkout = () => {
  ...
  const { exercises, clearContext, setWorkoutName, setExercises, workoutName } = useContext(CreateWorkoutContext);
      ...
      <ScrollView className="flex-1 px-4">
        ...
        <View className="mb-6 gap-4 pr-2 flex-row justify-between items-center w-full">
          ...
          <GenericButton
            ...
            onPress={async () => {
              ...
              let response;
              if (workoutId) {
                response = await fetch(`${process.env.EXPO_PUBLIC_BACKEND_URL}/workouts/update`, {
                  method: 'POST',
                  headers: {
              ...
              response = await fetch(`${process.env.EXPO_PUBLIC_BACKEND_URL}/workouts/create`, {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                  Authorization: `Bearer ${await SecureStore.getItemAsync('jwtToken')}`,
        ...
        {exercises.length > 0 && exercises.map((exercise, index) => (
          <SwipeableExerciseItem
            ...
            exercise={exercise}
            index={index}
            onDelete={() => setExercises(exercises.filter((_, i) => i !== index))}
          />
        ))}
